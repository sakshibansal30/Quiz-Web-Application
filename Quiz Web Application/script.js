...
/*chatIcon.onclick = ()=>{
	alert("ChatBox");
	if(chatBox.classList.toString().indexOf('active') == -1) {
...
function showResult(){
    ...
    if (userScore > 10){ // if user scored more than 10
        ...
        let scoreTag = '<span> Superb! , You got <p>'+ userScore +'</p> out of <p>'+ questions.length +'</p></span>';
...
login.onclick = (e)=>{
  ...
  if(!eField.classList.contains("error") && !pField.classList.contains("error")){
	  ...
	  for(var i = 0; i < loginIdPassword.length; i++) {
		  if(loginIdPassword[i].LoginId == eInput.value) {
			  if(loginIdPassword[i].Password == pInput.value) {
				  valid_login = true;
			  }
		  }
	  }
